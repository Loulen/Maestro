...
- **Instrumentation inégale, jamais silencieuse** : coût, résolution de transcript, substrat de fin de tour, détection de menu de limite, staging set, usage de contexte sont des **capacités** écrites harnais par harnais. Absente, la capacité rend la feature absente et le **dit** (« — », jamais `$0`) — ADR-0045.
...
- **Identité de session imposée** *(terme, #702)* : un harnais qui accepte un id de session **au lancement** et le **crée s'il n'existe pas** laisse PDO nommer la session avant qu'elle existe ; le transcript se résout alors par cet id, jamais par répertoire de travail, et deux nœuds concurrents dans le même worktree ne se confondent plus. `claude` et `copilot` le font ; `pi` aussi (`--session-id`, créé-ou-repris, même flag pour la reprise) ; `opencode` non (ADR-0045, limites). La reprise d'un tel harnais est **toujours par identité**, jamais aveugle (`--continue` rouvrirait la dernière session du cwd, la collision qu'on supprime). _Éviter_ : « session par cwd », « résolution par répertoire » pour un harnais qui impose l'identité.
- **Extension de fin de tour** *(terme, `pi`, ADR-0043 appliqué)* : l'équivalent pour `pi` du hook `Stop` de `claude` — une extension éphémère écrite par le daemon par nœud, chargée par `-e`, qui exécute `pdo complete --auto` sur l'événement `agent_settled`. Elle passe par le **trou `{settings}`** du descripteur (un « fichier de réglages injecté », même sens que pour `claude`), pas par un nouveau trou. Régie par le **même** réglage `autocomplete_turn_end`, **décoché par défaut** : décoché, aucun fichier n'est écrit, le token `-e` disparaît, l'agent fait `pdo complete` lui-même. Repli : balayage du tail de la session JSONL (`stopReason`). _Éviter_ : « mode json » comme substrat (mode driver, non résident) ; un second interrupteur propre à `pi`.
- **Coût rapporté en dollars** *(précision d'ADR-0052)* : quand le harnais calcule lui-même un coût **déjà en dollars** par message (`pi` : `usage.cost.total`), c'est un coût **rapporté** de constante de conversion **1.0**, sans `~`. Un message qui porte des **tokens sans coût** (catalogue absent du home : `pi` prévient et tourne) rend le total du nœud **indisponible** (« — »), jamais `$0` ; la ventilation reste (ADR-0052 §3). _Éviter_ : recalculer depuis les tokens ; lire `0` comme gratuit.
...
- **Le catalogue se lit d'abord dans la source *générée*** *(terme, ADR-0056)* : un binaire n'énumère pas forcément là où on regarde. Le catalogue a quatre sources, par préférence décroissante — le **script de complétion** (`<binaire> completion bash`, généré à partir des choix que le CLI déclare, donc préféré), la **liste de modèles** (`<binaire> --list-models`, table générée depuis le catalogue embarqué, avec la fenêtre de contexte par modèle — `pi`, #702), le **sujet d'aide des réglages** (`<binaire> help config`), puis `--help`. Chaque axe (modèles, efforts) appartient à la **source la mieux placée qui répond pour lui**. `--help` tourne en premier quand même, parce que c'est là qu'un CLI **déclare ses sous-commandes** : on n'exécute que celles qu'il annonce (`claude` n'a pas de `completion` et lit l'argv comme un prompt — la lancer à l'aveugle coûterait un timeout de cinq secondes dans une réponse `/settings`). Mesuré : le `--help` de copilot 1.0.80 énumère ses efforts mais décrit `--model` en prose — le lire seul faisait conclure « copilot n'a pas de catalogue » alors qu'il en a un (#629). _Éviter_ : « la source du catalogue » au singulier ; choisir la source par nom de harnais (l'échelle est harnais-agnostique).
...
- **Modèle observé / effort observé** *(termes, ADR-0065)* : la valeur que la **source du harnais** rapporte pour une exécution (modèle par message, effort quand la source le porte : `pi`, `copilot`), par opposition au **modèle demandé / effort demandé**, l'intention gelée dans l'événement de démarrage. Stats lit l'observé d'abord, le demandé en repli, et dit toujours la provenance. Un modèle est son **id verbatim**, une seule ligne quel que soit le harnais ; un alias épinglé et un id observé font deux lignes. _Éviter_ : « modèle du node » sans dire lequel des deux ; « modèle réel » (observé) ; normaliser ou fusionner des ids.
...
- **Skill seedé** (#722) : au démarrage du daemon, le skill `pdo-orchestrate` est écrit dans la banque — créé si absent, son contenu réécrit quand la copie embarquée dans le binaire diffère (le frontmatter porte `skill_version`, versionnée avec PDO) — dans un dossier « PDO », et **verrouillé** : ni édition ni suppression via la surface banque (`SkillError::Locked`, badge « Managed by PDO » côté UI). Un seul skill seedé — le mécanisme est minimal par décision d'ADR-0064. _Éviter_ : « skill du produit » (il vit dans la banque de l'instance comme tout skill, sélectionnable et livrable), « mise à jour de la banque » (le daemon ne réécrit que ce skill).
...
Surface **Stats** superposée en pleine fenêtre, avec navigation latérale et période commune aux sections : agrégats **transverses** filtrables par période (runs/erreurs/sessions, fires par pipeline, coûts), à distinguer des Statistiques de Run. Sessions et Cost ventilent les harnais dans des barres empilées par période et dans un tableau hiérarchique Pipeline → Nodes ; Cost affiche total et moyenne, tandis que Sessions porte les nombres d'exécutions. Les noms visibles suffisent, l'interface n'expose pas leurs identifiants techniques. Une lecture secondaire reprend la même hiérarchie par Projet → Pipelines → Nodes, et une troisième **par Modèle → effort → Pipelines → Nodes** (Cost et Performance, ADR-0065), le drill d'un Node se terminant par ses couples modèle × effort ; un Run sans Projet nommé porte le nom de son dépôt primaire. Les sessions propres au Run forment la ligne **Infrastructure**, tandis que les subagents appartiennent au Node parent (ADR-0058) ; un coût impossible à rattacher reste dans **Non attribué**, jamais dans une catégorie inventée. Une exécution de Node est un démarrage, donc chaque tour de boucle et chaque redémarrage compte séparément dans la moyenne. La période sélectionne les Runs démarrés dans sa fenêtre et inclut toutes leurs exécutions et leur coût complet. Une moyenne de coût ne porte que sur les exécutions au coût lisible ; elle garde le préfixe `~`, et son détail donne la couverture. Le marqueur `†` reste réservé à une borne basse incomplète. Un harnais n'est absent du tableau que s'il n'a aucune exécution sur la période ; un coût inconnu reste visible comme « — », jamais comme `$0`. Les événements antérieurs au multi-harnais sont attribués à `claude`, seul harnais agentique alors disponible ; les Nodes `script` restent hors compte. Les couleurs sont stables par nom de harnais. Le calcul ne se rafraîchit pas en arrière-plan : l'utilisateur déclenche **Refresh** et voit l'heure du dernier calcul. Les tarifs vivent dans le panneau secondaire **Pricing details**, avec leur synchronisation, la table résolue et ses avertissements. Les agrégats sont dérivés à la lecture, **jamais matérialisés** (ADR-0029), et restent exprimés en USD.

### Relecture de diff (Review)

**Onglet Diff** *(terme, #746)* : onglet de niveau Run — il remplace l'ancienne section repliable d'Info — qui montre, à la façon d'un terminal Warp, le **diff de Run** (trois-points depuis le **point de fork du Run (`fork_sha`)**, jamais depuis le `HEAD` du checkout partagé — #417 —, exclut `.pdo/`, mêmes bornes que LOC pour que « compté » et « montré » coïncident) en liste de fichiers repliables. Vue rapide, sans choix de refs ; un bouton **Expand and comment** mène à la page de Review. Le diff se calcule dans le **dépôt effectif du Run**, jamais dans le cwd du daemon (le bug qui le rendait invisible). Un Run archivé affiche « Diff not preserved for archived runs », à distinguer de « No changes ». _Éviter_ : « diff de nœud » comme surface (il n'y en a plus ; un nœud n'offre qu'un raccourci vers la Review avec sa livraison présélectionnée), « section Diff ».

**Page de Review** *(terme)* : page dédiée, à sa propre URL, qui rend un diff **entre deux refs du Run** à la façon d'une PR GitHub : fichiers à gauche, side-by-side par défaut (bascule unifiée), expansion du contexte entre hunks, et **commentaires de review** inline. Le rendu s'appuie sur un composant de diff tiers ; la valeur du produit est dans la conversation, pas dans le parseur de hunks. _Éviter_ : « vue expanded », « PR » (rien n'est poussé nulle part).

**Ref de Run** *(terme)* : un point git persistant connu du Run, proposé comme source ou destination de la Review : le **point de fork**, le **tip du Run**, et pour chaque nœud livré ses SHA **before/after** de livraison, libellés par le nom du nœud et son `iter`. Un nœud en cours y ajoute sa branche de sous-worktree vivante. Les branches de sous-worktree ne survivent pas au merge-back, donc la Review ne raisonne jamais « par branche de nœud » (ADR-0067). Défaut : point de fork → tip du Run. _Éviter_ : « branche du nœud » comme identité stable.

**Commentaire de review** *(terme)* : remarque ancrée sur une ligne d'un fichier (`path`, côté old/new, ligne, refs source et destination). États : **`draft`** (éditable, supprimable) → **`sent`** (envoyé au manager, immuable, événement du Run) → **`resolved`**, avec **`reopen`** possible. Quand la destination affichée a bougé, le commentaire est **reporté** si sa ligne est inchangée, sinon marqué **`outdated`** avec son hunk d'origine conservé — le comportement GitHub. Tout ce qui suit `sent` vit dans l'**event log** du Run et se lit post-mortem (ADR-0067). _Éviter_ : « annotation », « note » (= note de canvas), « issue ».

**Envoi au manager** *(terme)* : geste qui passe un ou plusieurs commentaires en `sent` et remet au Pipeline Manager **un seul message** contenant, par commentaire, son id, l'ancre, l'extrait du hunk, le texte, et la consigne de répondre via `pdo review reply`. Le manager est démarré à la demande s'il ne tourne pas ; l'envoi est grisé, raison affichée, quand la branche du Run n'existe plus. _Éviter_ : « notifier », « inject » (`inject_artifact` écrit un artefact, pas un message).
...
- **Surface Settings** (#688) : pleine fenêtre, même coquille que Stats — un **rail** de **catégories** (General, Agents, Sandbox & worktrees, Diagnostics) et, par catégorie, une page défilante découpée en **sous-catégories** listées dans une deuxième colonne qui sert d'ancre (scroll-spy). Les anciens drill-downs (profils agentiques, profils de staging, provisionnement) sont des sous-catégories inline dont chaque édition est écrite aussitôt (badge `saves as you go`, hors du Save du formulaire) ; **exception (#691, décision du 2026-09-04)** : la **Banque de skills** reste une surface à part, ouverte depuis la sous-catégorie Skills (« Open skill bank »), parce que parcourir, lire, importer et mettre à jour des skills est un parcours en soi, pas un champ de réglage. **New Run** ouvre Settings directement sur Sandbox & worktrees › Staging profiles (« Manage staging profiles… »), la section étant pulsée une fois ; New Run reste monté dessous et recharge ses profils à la fermeture. Un seul **Save** explicite pour le formulaire d'instance, visible partout ; changer de catégorie ne perd jamais une modification. Diagnostics est en lecture seule. _Éviter_ : « onglet » (c'est le vocabulaire de Stats, dont le rail n'a pas de second niveau), « modale de settings » (ce n'est plus une boîte de dialogue), « page de sous-catégorie » (une sous-catégorie est une section, pas un écran).
...
Liste **nommée** de ce qu'un Run sandboxé stage dans son home **en plus des staging sets** des harnais qu'il lance — plus son **env** et sa **source d'image** : un profil décrit le home, l'env **et** le conteneur, et il est **harnais-agnostique** (il ne sait pas ce qu'est un harnais, ADR-0063). Ce qui est stocké est un **diff** d'intention (`disabled`/`extras`), jamais un instantané — sinon une install ne verrait plus les évolutions futures du défaut. Le nom **et** la liste résolue sont **gelés au lancement du Run** : la préparation lit l'état du Run, jamais le réglage vivant. Pas de rename en v1 (le nom est aussi la valeur stockée par ses trois consommateurs). _Éviter_ : « allowlist » seul, « preset », « template » ; croire que le profil ne décrit que le home.

**Staging set** *(terme, par harnais, ADR-0063)* :
La liste, déclarée **en code par chaque harnais first-party**, des entrées de `$HOME` et des variables d'env qui font qu'une session dans le conteneur se comporte comme sur l'hôte : authentification, réglages, catalogue de modèles, extensions, skills (`claude` : l'allowlist de `~/.claude` ; `pi` : `.pi/agent/` sauf `sessions/`, plus `PI_SKIP_VERSION_CHECK=1`, `PI_TELEMETRY=0`). Une entrée marquée **transcripts** est exclue à l'aller et **rapatriée** au merge-back. C'est une **capacité** au sens d'ADR-0045/0051 : `None` est une valeur déclarée, publiée dans le tableau de support. Le set d'un harnais se copie **au spawn du premier nœud du Run qui le résout** — jamais au `RunStarted`, jamais pour un harnais que le Run ne lance pas ; la racine de home de chaque harnais first-party est montée **vide** à la création du conteneur pour le permettre. _Éviter_ : « plancher de staging » (l'ancien terme, qui ne désignait que les gestes de `claude`) ; « profil `pi` » (le profil est par Run, un Run lance plusieurs harnais) ; « home stagé de claude » comme synonyme (c'est un staging set parmi d'autres) ; croire qu'un staging set contient l'image (l'image n'est pas l'affaire de PDO).

**Autonomy fixups** *(terme, par harnais, ADR-0063)* :
Les écritures qui **désarment un dialogue bloquant** une fois le staging set copié : pour `claude`, confiance pré-accordée à la racine du Run, bypass permissions accepté, managed settings de l'org consentis — chacune satisfaite par une **copie** de l'hôte ou une **synthèse de repli**, ce qui rend le décochage d'une entrée sûr sans l'interdire. `pi` et `copilot` n'en ont **aucun** : leur argv porte le flag (`-a`, `--allow-all`). Trois fixups de `claude` ne sont pas des entrées du tout : affichés en lecture seule, refusés même en extra. _Éviter_ : « garanties » sans dire de quel harnais ; « fichiers obligatoires », « liste verrouillée ».
...
`prepare` seede le staged home depuis la liste gelée du profil (symlinks échappants déréférencés ; walk best-effort) et monte vide la racine de home de chaque harnais first-party → au **spawn** d'un nœud, le staging set et les autonomy fixups du harnais résolu sont posés s'ils ne l'ont pas encore été dans ce Run, et l'existence du binaire dans le conteneur est vérifiée (absent : dit une fois, nœud `Interrupted`) → les sessions tournent → **merge-back** : à la transition terminale du Run **et** à `cleanup_run`, seuls les **transcripts** (entrées marquées *transcripts* des staging sets : `*.jsonl` de `.claude/projects/`, `.pi/agent/sessions/`) sont recopiés vers l'hôte, sous le même dirname encodé (le worktree est monté au même chemin absolu, donc `pi` nomme le répertoire pareil dedans et dehors), de façon **idempotente** — aucune autre écriture ne revient vers l'hôte → `teardown` purge le staging dir. Le coût et la veille lisent les transcripts **du staging** tant qu'un Run sandboxé est vivant. Le staging n'est purgé qu'au `cleanup_run` (dette disque à surveiller). _Éviter_ : « sync », « flush » pour le merge-back ; « cleanup » pour le teardown (réservé au niveau Run).
